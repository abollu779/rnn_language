...
def kernel_ridge(X,Y,Xval,lmbda):
    clf = KernelRidge(alpha=lmbda)
    clf.fit(X, Y)
    return clf.predict(Xval), clf.predict(X)
...
def kernel_ridge_by_lambda(X, Y, Xval, Yval, lambdas=np.array([0.1,1,10,100,1000])):
    error = np.zeros((lambdas.shape[0],Y.shape[1]))
    for idx,lmbda in enumerate(lambdas):
        preds,_ = kernel_ridge(X,Y,Xval,lmbda)
        error[idx] = 1 - R2(preds,Yval)
    return error
...
def cross_val_ridge(train_features,train_data, test_features, n_splits = 10,
    ...
    for idx_lambda in range(lambdas.shape[0]): # this is much faster than iterating over voxels!
        ...
        preds[:,idx_vox], preds_train[:,idx_vox] = ridge_2(train_features, train_data[:,idx_vox], test_features, lambdas[idx_lambda])
    if do_plot:
        plt.figure()
        plt.imshow(weights,aspect='auto',cmap = 'RdBu_r',vmin = -0.5,vmax = 0.5);

    return preds,preds_train, np.array([lambdas[i] for i in argmin_lambda])
