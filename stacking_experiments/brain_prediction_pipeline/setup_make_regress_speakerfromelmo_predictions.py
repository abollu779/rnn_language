...
save_dir = '/share/volume0/abollu/neuromod/results'
...
for subject in cneuromod_subjects:
	for regress_feat_types in all_regress_feat_types:	
                ...
                qsubStr = 'qsub -l walltime=24:00:00 -l vmem="100gb" -p 0 -q pool2 -v repetition=%d,perm_shift=%d,perm_block=%d,predict_feat_type=%s,nlp_feat_type=%s,nlp_feat_dir=%s,layer=%d,sequence_length=%d,output_prefix=%s,delay=%d,regress_feat_types=%s,subject=%s,output_dir=%s run_make_regress_speakerfromelmo_predictions.sh' % (repetition,perm_shift,perm_block, predict_feat_type, nlp_feat_type, nlp_feat_dir, layer, seq_len, output_fname, delay, regress_feat_types, subject, output_dir)
                print(qsubStr)
                os.system(qsubStr)
