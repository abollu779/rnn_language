...
# Parses screenplay from word document and creates txt file of CHARACTER: DIALOGUE
...
def readScreenplay(movie_name):
    screenplay_path = 'screenplays/raw_docs/{}.docx'.format(movie_name)
...
def saveDialogues(dialogues, movie_name):
    with open("screenplays/parsed_character_lines/{}.txt".format(movie_name), 'w+') as f:
        i, n = 0, len(dialogues)
        while i < n:
            assert i+1 < n
